<template>
  <b-card body-class="p-0" header-class="border-0">
    ...
    <el-table class="table-responsive table" :data="tableData" header-row-class-name="thead-light">
      <el-table-column label="NFT" min-width="115px" prop="name">
        <template v-slot="{row}">
          <div class="font-weight-600">{{row.name}}</div>
        </template>
      </el-table-column>

      <el-table-column label="Name" min-width="115px" prop="name">
        <template v-slot="{row}">
          <div class="font-weight-600">{{row.name}}</div>
        </template>
      </el-table-column>

      <el-table-column label="Participation" min-width="110px" prop="visitors">
      </el-table-column>

      <el-table-column label="progress" min-width="220px" prop="progress">
        <template v-slot="{row}">
