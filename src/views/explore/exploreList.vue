    ...
    <div class="inftlist">
      <div class="inftlist-find bg_lightgray" :class="findPanelOpen?'inftlist-find-open':'ifind-header-close'">
        <div class="ifind-header" @click="changPanel">
          <div class="ititle" v-if="findPanelOpen"><img src="../../assets/imgs/findfilters.svg"
              class="ifileter-img" /><span>{{$t('page.filters')}}</span></div>
        ...
        <div class="ifind-body" v-if="findPanelOpen">
          ...
          <div class="ifrom-group">
            ...
            <div class="iinput-area iinput-area-find">
              <div class="icreate-input-outer">
                <input type="number" class="iinput iinput-find iinput-find-v2" v-model="form.min" placeholder="Min"
                  min="0" max="999999" />
              </div>
              <div class="ito">To</div>
              <div class="icreate-input-outer">
                <input type="number" class="iinput iinput-find iinput-find-v2" v-model="form.max" placeholder="MAX"
                  min="0" max="999999" />
              </div>
            </div>
          </div>
...
<script>
  import exploreFindLists from '../../components/nfts/exploreFindLists.vue'
  export default {
    name: 'explorelist',
    components: {
      exploreFindLists
    ...
    data() {
      return {
        ...
        stats: [{
            text: this.$t('page.filters_state_all'),
            value: 'All'
          },
        {
            text: this.$t('page.filters_state_completed'),
            value: 'Completed'
          },
          {
            text: this.$t('page.filters_state_inprogress'),
            value: 'In progress'
          },
          {
            text: this.$t('page.filters_state_reward'),
            value: 'Reward'
          }
        ],
        rewards: [{
            text: this.$t('page.filters_reward_all'),
            value: 'All'
          },
          {
            text: this.$t('page.filters_reward_nft'),
            value: 'NFT'
          },
          {
            text: this.$t('page.filters_reward_eth'),
            value: 'ETH'
          }
        ],
      }
    ...
    methods: {
      changPanel() {
        this.findPanelOpen = !this.findPanelOpen
      }
    },
  }
</script>
...
<style scoped>
  ...
  .ifind-panel-header .iexpansion-img {
    width: 1.222222rem;
    height: 1.111111rem;
    margin: 1.111111rem;
  }

  .ifind-panel-header .iexpansion-img:hover {
    opacity: .6;
  }

  .ifind-panel-header .transform-180 {
    transform: rotate(180deg);
    float: right;
  }

  .ifind-panel-header-title {
    height: 3.84rem;
    overflow: hidden;
  ...
  .idate-select-contain .idate-select {
    width: 7.333333rem;
    height: 2.666666rem;
    border: 2px solid #EEEEEE;
  }
  ...
  .iexplore-right {
    width: -webkit-calc(100% - 23.388888rem);
    margin: 0 auto;
  }
  ...
  /* 2+20+22+20+2, 100%-(2+20+22+20+2)-30-30 */
  .iexplore-right2 {
